...
if(isset($_POST['send'])){
	...
	if(DB::query("SELECT id FROM users WHERE id = :reciever",array(":reciever"=>$_GET['reciever']))){
		...
		header("LOCATION: talk.php?username=". urlencode($_GET['username']));
...
<body>
<a href="full-talk.php?username=<?php echo $_GET['username']; ?>" class="button"><i class='material-icons'>refresh</i></a>
